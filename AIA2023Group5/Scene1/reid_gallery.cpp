...
EmbeddingsGallery::EmbeddingsGallery(const std::string& ids_list,
    ...
    try 
    {
        std::ifstream ifs(ids_list); // open the JSON file
        Json::Value root; // define a Json::Value object to hold the parsed JSON data
        ifs >> root; // parse the JSON data from the file into the root object

        // iterate over the keys in the root object and extract the values

        std::vector<cv::Mat> embeddings;
        int id = 0;
        for (Json::ValueIterator itr = root.begin(); itr != root.end(); ++itr) {
            std::string key = itr.key().asString(); // get the key as a string
            std::string value = (*itr)[0].asString(); // get the first value in the array as a string
            std::cout << key << " = " << value << std::endl; // print the key-value pair
            ...
            if (file_exists(value)) {
                ...
                path = folder_name(ids_list) + separator() + value;
            }

            cv::Mat image = cv::imread(path);
            CV_Assert(!image.empty());
            cv::Mat emb;
            RegistrationStatus status = RegisterIdentity(key, image, min_size_fr, crop_gallery, detector, landmarks_det, image_reid, emb);
            if (status == RegistrationStatus::SUCCESS) {
                embeddings.push_back(emb);
                idx_to_id.push_back(id);
                identities.emplace_back(embeddings, key, id);
                ++id;
            }
        }
    }
...
RegistrationStatus EmbeddingsGallery::RegisterIdentity(const std::string& identity_label,
    ...
    cv::Mat landmarks;
    //** if target dont use crop_face it will cause bug in debug mode  **
